...
class Graph implements NodeFactory {
	...
	/**
	 * Field "nodes" is a Map because we want to map each node to its NEXT field! This makes it faster to search for the
	 * node with given NEXT field in the method exists(Node node).
	 */
	...
	public int index_equivalence(final Node node, final Node[] equivalence) {
		...
		for (index = 0; index < size; index++) {
			if (equivalence[index] == null) {
				break;
			} else if (equivalence[index].getNext().equals(node.getNext())) { return equivalence[index].getId(); }
		}
