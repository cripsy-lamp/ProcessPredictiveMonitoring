...
public class PartitionedConjunction extends UnsortedTreeConjunction {
	...
	public PartitionedConjunction(final TreeFactory<ConjunctionTreeNode, ConjunctionTreeLeaf> treefactory,
	        final ConjunctionFactory<? extends UnsortedTreeConjunction> factory, final Collection<Formula> terms) {
		super(treefactory);
	...
	public synchronized void setAll(final Collection<Formula> formulas) {
		reset();
		Map<ConjunctionTreeNode, Automaton> cache = new HashMap<ConjunctionTreeNode, Automaton>();
	...
	private List<Set<Formula>> mergePartitions(Map<ConjunctionTreeNode, Automaton> cache, final Formula formula) {
	...
	private Collection<String> getAtomicPropositions(Map<ConjunctionTreeNode, Automaton> cache, final Formula formula) {
...
//		}
	...
	public synchronized void add(final Formula term) throws OperationNotSupportedException {
		...
		if (oldPartitions.size() > 1) {
			...
			if (child != null) {
				ConjunctionTreeNode oldRoot = child.root;
				child.add(term);
	...
	public synchronized void remove(final Formula term) throws OperationNotSupportedException {
		...
		final Set<Formula> partition = partitions.get(FormulaTools.getAtomicPropositions(term).iterator().next());
	...
	private void replaceRoot(final UnsortedTreeConjunction child, ConjunctionTreeNode oldRoot) {
		ConjunctionTreeNode newRoot = child != null?child.root:null;
		if (oldRoot != newRoot) {
			ConjunctionTreeNode parent = roots.get(oldRoot);
			if (parent == null) {
				if (root == oldRoot)
				root = newRoot;
				else if (newRoot != null) addChild(child);
				assert roots.isEmpty();
			} else {
				if (newRoot == null) {
					ConjunctionTreeNode gparent = parent.getParent();
					ConjunctionTreeNode other = other(oldRoot);
					if (gparent != null) {
	...
	private void addChild(final UnsortedTreeConjunction child) {
		...
		if (root == null) {
			...
			ConjunctionTreeNode newRoot = getTreeFactory().createNode(root, child.root);
	...
	private void mergePartitions(Map<ConjunctionTreeNode, Automaton> cache, final Set<Formula> last, final Set<Formula> partition) {
		final UnsortedTreeConjunction oldChild = children.remove(last);
