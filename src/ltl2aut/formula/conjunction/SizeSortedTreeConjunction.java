...
/**
 * Note: All the hard work happens when inserting into the sorted set (the comparator extracts the automaton).
 * 
 * @author mwesterg
 */
public class SizeSortedTreeConjunction extends UnsortedTreeConjunction {
	...
	public SizeSortedTreeConjunction(final TreeFactory<ConjunctionTreeNode, ConjunctionTreeLeaf> treefactory,
	...
	private static class NodeComparator implements Comparator<ConjunctionTreeNode> {
		public int compare(final ConjunctionTreeNode arg0, final ConjunctionTreeNode arg1) {
			...
			if (result == 0 && arg0 != arg1) {
				result = arg0.getAutomaton().getTransitionCount() - arg1.getAutomaton().getTransitionCount();
				if (result == 0) { return arg0.getAutomaton().hashCode() - arg1.getAutomaton().hashCode(); }
			}
			return result;
		}
	}
	...
	public synchronized void setAll(final Collection<Formula> formulas) {
		...
		final SortedSet<ConjunctionTreeNode> f = new TreeSet<ConjunctionTreeNode>(new NodeComparator());
	...
	private SortedSet<ConjunctionTreeNode> makePowerOfTwo(final SortedSet<ConjunctionTreeNode> f) {
		final SortedSet<ConjunctionTreeNode> result = new TreeSet<ConjunctionTreeNode>(new NodeComparator());
		int i = 1;
		while (i < f.size()) {
			i *= 2;
		}
