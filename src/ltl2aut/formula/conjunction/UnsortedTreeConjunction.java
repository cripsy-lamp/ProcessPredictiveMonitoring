...
public class UnsortedTreeConjunction implements Conjunction {
	...
	public UnsortedTreeConjunction(final TreeFactory<ConjunctionTreeNode, ConjunctionTreeLeaf> treefactory) {
	...
	public synchronized void setAll(final Collection<Formula> formulas) {
		...
		for (final Formula f : formulas) {
			try {
				...
				assert false;
				// If subclasses remove implementation of add, they should add another implementation of setAll
			}
		}
	}
	...
	public synchronized void add(final Formula term) throws OperationNotSupportedException {
		...
		if (leaves.isEmpty()) {
			...
			if (root == original) {
				...
				leaves.add(original);
				leaves.add(newLeaf);
				replace(original.getParent(), original, getTreeFactory().createNode(original, newLeaf));
			}
		}
	}
	...
	private int getNumberCachedAutomata(final ConjunctionTreeNode elm) {
		...
		if (elm.cache != null && elm.cache.get() != null) { return 1 + getNumberCachedAutomata(elm.getLeft())
	...
	private int getMaxStates(final ConjunctionTreeNode elm) {
		if (elm == null) { return 0; }
		Automaton a;
		if (elm.cache != null && (a = elm.cache.get()) != null) { return Math.max(a.getStateCount(), Math.max(
		        getMaxStates(elm.getLeft()), getMaxStates(elm.getRight()))); }
	...
	private int getMaxEvents(final ConjunctionTreeNode elm) {
		if (elm == null) { return 0; }
		Automaton a;
		if (elm.cache != null && (a = elm.cache.get()) != null) { return Math.max(a.getTransitionCount(), Math.max(
		        getMaxEvents(elm.getLeft()), getMaxEvents(elm.getRight()))); }
	...
	private int getTotalStates(final ConjunctionTreeNode elm) {
		if (elm == null) { return 0; }
		Automaton a;
		if (elm.cache != null && (a = elm.cache.get()) != null) { return a.getStateCount()
		        + getTotalStates(elm.getLeft()) + getTotalStates(elm.getRight()); }
	...
	private int getTotalEvents(final ConjunctionTreeNode elm) {
		if (elm == null) { return 0; }
		Automaton a;
		if (elm.cache != null && (a = elm.cache.get()) != null) { return a.getTransitionCount()
		        + getTotalEvents(elm.getLeft()) + getTotalEvents(elm.getRight()); }
