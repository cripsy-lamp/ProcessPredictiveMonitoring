...
public class DFAOperations extends NDFAOperations {
	...
	/**
	 * Return an automaton accepting the same language as a, but where every transition is fully specified. The
		...
		for (final State s : a) {
			...
			for (final Transition t : s.getOutput()) {
				if (t.isAll()) {
					labels.clear();
					nlabels.clear();
					factory.addTransition(stateMap.get(s), stateMap.get(t.getTarget()), t.getLabel());
			...
			if (!labels.isEmpty()) {
				...
				for (final String label : labels) {
					factory.addPropositionTransition(stateMap.get(s), dead = getDead(dead, factory, id), label);
				}
			}
			if (!nlabels.isEmpty()) {
				...
				assert !negativeSeen;
				factory.addNegativePropositionsTransition(stateMap.get(s), dead = getDead(dead, factory, id), nlabels);
			}
	...
	/**
	 * Construct the synchronized product of the two automata (i.e., the automaton accepting the intersection of the
	 * languages accepted by the two automata).
	 * 
	 * @param a
	 * @param b
	 * @return
	 */
	public DeterministicAutomaton intersect(final DeterministicAutomaton a, final DeterministicAutomaton b) {
		return buildProduct(a, b, true);
	}

	/**
	 * Return a (possibly smaller) automaton accepting the same language as a. The parameter is not modified. TODO - use
	 * minimizeSimple for now (worse execution time). Input must be completed.
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * Return a (possibly smaller) automaton accepting the same language as a. The parameter is not modified. Input must
	 * be completed.
	 * 
	 * @param a
	 * @return
		...
		do {
			...
			for (final List<Integer> classMembers : new ArrayList<List<Integer>>(reverseMap)) {
				...
				if (subClasses.size() > 1) {
					...
					boolean first = true;
					for (final Map.Entry<Map<Integer, Integer>, List<Integer>> subClass : subClasses.entrySet()) {
						if (!first) {
		...
		for (int i = 0; i < nextClass; i++) {
			...
			for (final Map.Entry<Integer, Integer> transition : transitions.entrySet()) {
				if (transition.getKey() != 0) {
					...
					labels.add(label);
					factory.addPropositionTransition(newStates[i], newStates[transition.getValue()], label);
				}
			}
	...
	/**
	 * Return an automaton accepting the complement of the language accepted by a. The parameter is not modified.
	 * Assumes the automaton is completed (i.e., call complete if you are not sure).
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * Construct the synchronized product of the two automata (i.e., the automaton accepting the union of the languages
	 * accepted by the two automata).
	 * 
	 * @param a
	 * @param b
	 * @return
	 */
	public DeterministicAutomaton union(final DeterministicAutomaton a, final DeterministicAutomaton b) {
		return buildProduct(a.op.complete(), b.op.complete(), false);
	}

	private DeterministicAutomaton buildProduct(final DeterministicAutomaton a, final DeterministicAutomaton b,
