...
public class NDFAOperations {
	...
	/**
	 * Return a deterministic automaton accepting the same language as a. The parameter is not modified. Beware that in
	 * the worst case the size of the result is exponential in the size of the input. Assumes the labels are either a
	 * single positive proposition, a list of negative propositions only, or a true proposition. This is the format
	 * emitted by LTL2BA (setting single to true). The automaton returned also satisfies this requirement.
	 * 
	 * @param a
	 * @return
	 */
				...
				for (final String label : labels) {
					...
					for (final State state : states) {
						for (final Transition transition : state.getOutput()) {
							...
							} else if (transition.isPositive() && transition.getPositiveLabel().equals(label)) {
								successor.add(transition.getTarget());
							} else if (transition.isNegative() && !transition.getNegativeLabels().contains(label)) {
								successor.add(transition.getTarget());
							}
						}
					}
	...
	/**
	 * Return an automaton accepting the intersection of the languages of a1 and a2. The parameters are modified.
	 * Creates labels, l1, l2, .., ln and !l1 && !l2 && .. && !ln
	 * 
	 * @param a1
	 * @param a2
	 * @return
	...
	/**
	 * Returns if this automaton definitely accepts all strings (i.e., if it return true, the automaton accepts all
	 * strings, if it returns false, it most likely doesn't).
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * Returns if this automaton definitely accepts the empty language (i.e., if it return true, the automaton accepts
	 * the ampty language, if it returns false, it most likely doesn't).
	 * 
	 * @param a
	 * @return
	 */
	...
	/**
	 * Return a (possibly smaller) automaton accepting the same language as a. The parameter is not modified. TODO
	 * 
	 * @param a
	 * @return
	 */
...
//	}
	...
	/**
	 * Return an automaton accepting the complement of the language accepted by a. The parameter is modified. TODO -
	 * determinize and use deterministic operation for now
	 * 
	 * @param a
	 * @return
	 */
...
//	}
	...
	/**
	 * Return an automaton accepting the union of the languages of a1 and a2. The parameters are modified. FIXME merges
	 * init states; should create new init state which is merged. THIS IMPLEMENTATION DOES NOT WORK!
	 * 
	 * @param a1
	 * @param a2
	 * @return
	...
	private Object createState(final AutomatonFactory factory, final int id, final Pair pair, final boolean conjunction) {
		final Object state = factory.createState();
		boolean accepting;
		if (conjunction) {
	...
	private Object createState(final AutomatonFactory factory, final Set<State> successor, final int id) {
	...
	private Object getStored(final Map<Set<State>, Object> storage, final Map<Set<State>, Object> accepting,
	...
	private Object registerNewState(final AutomatonFactory factory, final Map<Set<State>, Object> storage,
	        final Map<Set<State>, Object> accepting, final Set<State> state, final int id) {
	...
	protected void copyState(final AutomatonFactory factory, final Map<State, Object> stateMap, final State s,
	        final int id) {
		final Object ss = factory.createState();
	...
	protected void copyTransition(final AutomatonFactory factory, final Map<State, Object> stateMap, final Transition t) {
		factory.addTransition(stateMap.get(t.getSource()), stateMap.get(t.getTarget()), t.getLabel());
	}

	protected void copyTransitions(final Automaton a, final AutomatonFactory factory, final Map<State, Object> stateMap) {
